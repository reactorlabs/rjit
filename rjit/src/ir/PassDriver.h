...
template <typename aPass> struct PassDriver : public FunctionPass {
  public:
    static char ID;
...
template <typename aPass> char PassDriver<aPass>::ID = 0;

template <typename aPass>
PassDriver<aPass>::RegisterMe<PassDriver<aPass>> PassDriver<aPass>::Registered;

template <typename Pass> class ForwardDriver : public PassDriver<Pass> {
  public:
    Pass pass;

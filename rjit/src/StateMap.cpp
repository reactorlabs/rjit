...
std::vector<Value*>& StateMap::getValuesToSetAtLPad(Instruction* LPad) {
    ...
    if (it != valuesToSetAtLPadCache.end())
        return it->second;
    ...
    std::pair<ValuesToSetCache::iterator, bool> ret =
        valuesToSetAtLPadCache.insert(
            std::pair<Instruction*, std::vector<Value*>>(LPad,
                                                         std::move(*vec)));
...
std::vector<Value*> StateMap::getValuesToFetchAtOSRSrc(Instruction* OSRSrc,
    ...
    LocPairInfoMap::iterator LPIMIt =
        locPairInfoMap.find(LocPair(OSRSrc, LPad));
    if (LPIMIt != locPairInfoMap.end()) {
        ...
        if (lpInfo->globalCompCode != nullptr) {
            for (CompCodeArgs::iterator
                     it = lpInfo->globalCompCode->args->begin(),
                     end = lpInfo->globalCompCode->args->end();
                 it != end; ++it) {
    ...
    for (Value* valueToSet : valuesToSetAtDest) {
        ...
        if (lpInfo != nullptr) {
            // check for oneToOneValue or compensation code specific for this
            // pair
            LocPairInfo::ValueInfoMap::iterator VIMIt =
                lpInfo->valueInfoMap.find(valueToSet);
            if (VIMIt != lpInfo->valueInfoMap.end()) {
                ...
                if (VI->isOneToOneValue()) {
                    ...
                    for (CompCodeArgs::iterator
                             it = VI->compCode->args->begin(),
                             end = VI->compCode->args->end();
                         it != end; ++it) {
                        valuesToFetch.push_back(*it);
                    }
                }
        ...
        OneToOneValueMap::iterator oneToOneIt =
            defaultOneToOneMap.find(valueToSet);
        ...
        llvm::report_fatal_error(
            "[getValuesToFetchFromSrcFunction] could not "
    ...
    valuesToFetch.erase(std::unique(valuesToFetch.begin(), valuesToFetch.end()),
                        valuesToFetch.end());

    return valuesToFetch;
}

std::pair<BasicBlock*, ValueToValueMapTy*>
StateMap::genContinuationFunctionEntryPoint(
    ...
    if (lpInfo != nullptr && lpInfo->globalCompCode != nullptr) {
        ...
        lastCreatedBB = addGlobalCompensationCode(
            lastCreatedBB, lpInfo->globalCompCode, fetchedValuesToOSRContArgs);
    }
    ...
    for (Value* dest_v : valuesToSetAtOSRCont) {
        if (lpInfo != nullptr) {
            ...
            // pair
            LocPairInfo::ValueInfoMap::iterator VIMIt =
                lpInfo->valueInfoMap.find(dest_v);
            if (VIMIt != lpInfo->valueInfoMap.end()) {
                ValueInfo* VI = VIMIt->second;
                if (VI->isOneToOneValue()) {
                    Value* src_v = VI->oneToOneValue;
                    (*updatedValuesToUse)[dest_v] =
                        fetchedValuesToOSRContArgs[src_v];
                    continue;
                } else {
                    lastCreatedBB = addLocalCompensationCode(
                        lastCreatedBB, dest_v, VI, updatedValuesToUse,
        ...
        dest_v->dump();
        llvm::report_fatal_error(
            "[genContinuationFunctionEntryPoint] missing "
...
BasicBlock* StateMap::addGlobalCompensationCode(
    BasicBlock* curBlock, StateMap::CompCode* compCode,
...
BasicBlock* StateMap::addLocalCompensationCode(
    BasicBlock* curBlock, Value* dest_v, StateMap::ValueInfo* valInfo,
    ValueToValueMapTy* updatedValuesToUse,
    ValueToValueMapTy& fetchedValuesToNewDestArgs) {
...
void StateMap::registerOneToOneValue(Value* src_v, Value* dest_v,
                                     bool bidirectional) {
...
void StateMap::registerLandingPad(Instruction* OSRSrc, Instruction* LPad,
                                  bool bidirectional) {
...
StateMap::LocPairInfo* StateMap::createLocPairInfo(StateMap::LocPair& pair) {
    ...
    std::pair<LocPairInfoMap::iterator, bool> ret = locPairInfoMap.insert(
        std::pair<LocPair, LocPairInfo>(pair, LocPairInfo()));

...
Value* Compiler::compileICCallStub(Value* call, Value* op,
    ...
    auto res = CallInst::Create(ic_stub, ic_args, "", b.blockSentinel()->first());
    AttributeSet PAL;
    {
...
Value* Compiler::compileCondition(SEXP e) {
    ...
    b.setBlock(next);
    PHINode* phi = PHINode::Create(t::SEXP, 2, "", b.blockSentinel()->first());
...
Value* Compiler::compileForLoop(SEXP ast) {
    ...
    b.setBlock(forCond);
    PHINode* control = PHINode::Create(t::Int, 2, "loopControl", b.blockSentinel()->first());
    ...
    BranchInst::Create(forBody, b.breakTarget(), test->result(), b.blockSentinel()->first());
...
Value* Compiler::compileSwitch(SEXP call) {
    ...
    BranchInst::Create(switchCharacter, switchIntegral, cond->result(), b.blockSentinel()->first());
    ...
    PHINode* result = PHINode::Create(t::SEXP, caseAsts.size(), "", b.blockSentinel()->first());

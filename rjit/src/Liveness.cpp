...
LivenessAnalysis::LiveValues LivenessAnalysis::analyzeLiveInForSeq(
    const BasicBlock* B, const LivenessAnalysis::LiveValues& liveOut,
    const Instruction* first, const Instruction* last) {
    // initially LIVE_IN = LIVE_OUT
    LiveValues liveIn(liveOut);

    BasicBlock::InstListType::const_reverse_iterator revIt =
        B->getInstList().rbegin();
...
bool LivenessAnalysis::processBlock(
    ...
    LiveValues currInValues =
        analyzeLiveInForSeq(B, outValues, &*firstNonPHIInstr);

    // remove PHI nodes created at this block from the set: we do not want to
    // propagate them to a predecessor of B
    for (BasicBlock::const_iterator it = firstInstr; it != firstNonPHIInstr;
         ++it) {
...
void LivenessAnalysis::run() {
    ...
    do {
        hasChanged = false;
        for (Function::BasicBlockListType::const_reverse_iterator
                 revIt = blocks.rbegin(),
                 revEnd = blocks.rend();
             revIt != revEnd; ++revIt) {
...
LivenessAnalysis::getLiveInValues(const BasicBlock* B) {
    return blockMap[B].first;
}

LivenessAnalysis::LiveValues&
LivenessAnalysis::getLiveOutValues(const BasicBlock* B) {
    return blockMap[B].second;
}

void LivenessAnalysis::updateLiveValuesForBlock(
    const llvm::BasicBlock* B, LivenessAnalysis::LiveValues&& liveIn,
    LivenessAnalysis::LiveValues&& liveOut) {
    blockMap[B] = std::pair<LiveValues, LiveValues>(std::move(liveIn),
                                                    std::move(liveOut));
}

std::ostream& operator<<(std::ostream& sin,
                         const LivenessAnalysis::LiveValues& values) {

...
class LivenessAnalysis {
  public:
    ...
    /// \brief Constructor
    LivenessAnalysis(llvm::Function* F) : F(F) { run(); }
    ...
    static LiveValues
    analyzeLiveInForSeq(const llvm::BasicBlock* B, const LiveValues& liveOut,
    ...
    void updateLiveValuesForBlock(const llvm::BasicBlock* B,
                                  LiveValues&& liveIn, LiveValues&& liveOut);

    /// \brief Print a LiveValues object to an output stream
    friend std::ostream& operator<<(std::ostream& sin,
                                    const LivenessAnalysis::LiveValues& values);

  private:
    typedef std::pair<LiveValues, LiveValues> LiveInAndOutValues;
    typedef std::map<const llvm::BasicBlock*, LiveInAndOutValues>
        BlockToLiveValuesMap;

    llvm::Function* F;
    BlockToLiveValuesMap blockMap;

    void run();
    bool processBlock(const llvm::BasicBlock* B, LiveValues& inValues,
                      const LiveValues& outValues);
};

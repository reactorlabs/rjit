...
/// the front-end, performs an indirect call to the function returned by the
/// code
...
class OSRLibrary {
  public:
    ...
    typedef struct OSRPointConfig {
        ...
        OSRPointConfig(bool verbose = false, bool updateF1 = true,
            ...
            : verbose(verbose), updateF1(updateF1),
              branchTakenProb(branchTakenProb), nameForNewF1(nameForNewF1),
              modForNewF1(modForNewF1), ptrForF1NewToF1Map(ptrForF1NewToF1Map),
              nameForNewF2(nameForNewF2), modForNewF2(modForNewF2),
              ptrForF2NewToF2Map(ptrForF2NewToF2Map) {
            if (branchTakenProb != -1 &&
                (branchTakenProb > 100 || branchTakenProb < 0)) {
                llvm::report_fatal_error("OSR probability should be either -1 "
                                         "or an integer number between 0 and "
                                         "100");
            }
        };
    } OSRPointConfig;
    ...
    typedef void* (*DestFunGenerator)(llvm::Function* F1,
                                      llvm::Instruction* OSRSrc, void* extra,
                                      void* profDataAddr);
    ...
    static OSRPair insertResolvedOSR(llvm::LLVMContext& Context,
                                     llvm::Function& F1,
    ...
    genContinuationFunc(llvm::LLVMContext& Context, llvm::Function& F1,
                        llvm::Function& F2, llvm::Instruction& OSRSrc,
                        llvm::Instruction& LPad,
                        std::vector<llvm::Value*>& valuesToPass, StateMap& M,
                        const std::string* F2NewName, bool verbose = false,
                        StateMap* *ptrForF2NewToF2Map = nullptr);
    ...
    getLiveValsVecAtInstr(const llvm::Instruction* I, LivenessAnalysis& LA);
  ...
  private:
    ...
    static void fixOperandReferencesFromVMap(llvm::Function* NF,
                                             llvm::Function* F,
                                             llvm::ValueToValueMapTy& VMap);
    static void replaceUsesAndFixSSA(
        llvm::Function* OSRCont, llvm::Instruction* OSRContLPad,
        std::vector<llvm::Value*>& valuesToSetAtOrigLPad,
        llvm::ValueToValueMapTy& VMap, llvm::ValueToValueMapTy& updatesForVMap,
        llvm::SmallVectorImpl<llvm::PHINode*>* insertedPHINodes, bool verbose,
        StateMap** ptrForF2NewToF2Map);
    static llvm::Function* duplicateFunction(llvm::Function* F,
                                             const llvm::Twine& Name,
                                             llvm::ValueToValueMapTy& VMap);
    static OSRCond regenerateOSRCond(OSRCond& cond,
                                     llvm::ValueToValueMapTy& VMap);
    static llvm::BasicBlock*
    generateTriggerOSRBlock(llvm::LLVMContext& Context, llvm::Function* OSRDest,
                            std::vector<llvm::Value*>& valuesToPass);
    static llvm::BasicBlock*
    insertOSRCond(llvm::LLVMContext& Context, llvm::Function* F,
                  llvm::Instruction* OSRSrc, llvm::BasicBlock* FireOSRBlock,
                  OSRCond& cond, const llvm::Twine& BBName,
                  int branchTakenProb);
    static void
    printLiveVarInfoForDebug(LivenessAnalysis::LiveValues& liveIn,
                             LivenessAnalysis::LiveValues& liveOut,
                             std::vector<llvm::Value*>& valuesToFetch);
    static LivenessAnalysis::LiveValues
    getLiveValsAtInstr(const llvm::Instruction* I, LivenessAnalysis& LA);
    static bool isConstantUsedInFunction(llvm::Constant* C, llvm::Function* F);
    static llvm::GlobalVariable* getVisibleDeclaration(llvm::GlobalVariable* G,
                                                       llvm::Module* M);

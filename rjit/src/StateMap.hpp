...
class StateMap {
  public:
        ...
        for (llvm::ValueToValueMapTy::const_iterator it = VMap->begin(),
            ...
            if (llvm::isa<llvm::Argument>(src_v)) {
                registerOneToOneValue(src_v, dest_v, bidirectional);
                           ...
                           llvm::dyn_cast<llvm::Instruction>(src_v)) {
                registerOneToOneValue(src_v, dest_v, bidirectional);
                if (!llvm::isa<llvm::PHINode>(src_I)) {
                    registerLandingPad(src_I,
                                       llvm::cast<llvm::Instruction>(dest_v),
                                       bidirectional);
                }
            }
        }
    ...
    StateMap(llvm::Function* F1, StateMap* sm1, llvm::Function* F2,
        ...
        : F1(F1), F2(F2), F1_LA(LivenessAnalysis(F1)),
          F2_LA(LivenessAnalysis(F2)) {
        // use getAllCorrespondingOneToOneValues.
        assert((F1 && F2 && sm1 && sm2) && "StateMap: null arguments.");

        auto default1 = sm1->getAllCorrespondingOneToOneValues();
        auto default2 = sm2->getAllCorrespondingOneToOneValues();
        ...
        for (auto i1 = default1.begin(), i2 = default2.begin();
            ...
            if (llvm::isa<llvm::Argument>(src_v)) {
                ...
                if (!llvm::isa<llvm::PHINode>(src_I)) {
                    registerLandingPad(
                        src_I, llvm::cast<llvm::Instruction>(dest_v), true);
                }
            }
        }
    }
    ...
    /// (\a OSRSrc, \a LPad) might also need Value objects not live at \a
    /// OSRSrc:
    ...
    getValuesToFetchAtOSRSrc(llvm::Instruction* OSRSrc,
    ...
    genContinuationFunctionEntryPoint(
        llvm::LLVMContext& Context, llvm::Instruction* OSRSrc,
        llvm::Instruction* LPad, llvm::Instruction* OSRContLPad,
    ...
    void registerOneToOneValue(llvm::Value* src_v, llvm::Value* dest_v,
                               bool bidirectional = false);
    ...
    void registerLandingPad(llvm::Instruction* OSRSrc, llvm::Instruction* LPad,
                            bool bidirectional = false);
    ...
    LocMap& getAllLandingPads() { return landingPadMap; }
    ...
    static std::pair<llvm::Function*, StateMap*>
    generateIdentityMapping(llvm::Function* F);
  ...
  private:
    ...
    // helper methods
    static void updateRefsToArgs(CompCode* compCode,
                                 llvm::ValueToValueMapTy& VMap);
    llvm::BasicBlock* addLocalCompensationCode(
        llvm::BasicBlock* curBlock, llvm::Value* dest_v, ValueInfo* valInfo,
        llvm::ValueToValueMapTy* updatedValuesToUse,
        llvm::ValueToValueMapTy& fetchedValuesToNewDestArgs);
    llvm::BasicBlock* addGlobalCompensationCode(
        llvm::BasicBlock* curBlock, CompCode* compCode,
        llvm::ValueToValueMapTy& fetchedValuesToNewDestArgs);
};
